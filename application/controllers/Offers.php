...
class Offers extends CI_Controller
{
    ...
    public function submit()
    {
        ...
        $this->session->set_flashdata(
            "success_msg",
            "تم إنشاء العرض وإرساله مباشرة لمدير الموارد البشرية (2230) للاعتماد.",
        );
    ...
    public function sign_commencement_digital()
    {
        ...
        // Process Image
        [$type, $sig_data] = explode(";", $sig_data);
        [, $sig_data] = explode(",", $sig_data);
    ...
    public function send_to_candidate($offer_id)
    {
        ...
        if (!$offer) {
            $this->session->set_flashdata(
                "error_msg",
                "CRITICAL ERROR: Could not find candidate linked to Application.",
            );
            redirect("offers/view/" . $offer_id);
            return;
        }
        ...
        if (!in_array($offer["status"], $allowed_statuses)) {
            $this->session->set_flashdata(
                "error_msg",
                "لا يمكن إرسال العرض إلا إذا كان معتمداً أو مرسلاً مسبقاً. الحالة الحالية: " . $offer["status"],
            );
            redirect("offers/view/" . $offer_id);
            return;
        }
        ...
        if ($result === true) {
            $success_text =
                $offer["status"] == "Sent" ? "تم إعادة إرسال العرض بنجاح عبر SMS." : "تم إرسال العرض بنجاح عبر SMS.";
    ...
    public function sign_commencement($offer_id, $role)
    {
        ...
        if ($role == "manager") {
            if (
                !in_array($uid, [1526, 1291, 64, 2200, 2439]) &&
                $this->session->userdata("role") != "recruitment_manager"
            ) {
    ...
    public function send_docs_request($offer_id)
    {
        ...
        if ($uid != 1526 && $uid != 64 && $uid != 2200 && $uid != 2439) {
    ...
    public function verify_docs_process($offer_id)
    {
        ...
        $this->db->query(
            "INSERT IGNORE INTO offer_doc_approvals (offer_id, user_id, created_at) VALUES (?, ?, NOW())",
            [$offer_id, $uid],
        );
    ...
    private function _send_sms_api($phone, $message, $app_id, $type)
    {
        ...
        try {
            ...
            $file_log =
                date("Y-m-d H:i:s") .
                " | App: $app_id | Phone: $phone | Type: $type | Status: $status | User: $user_id\n";
