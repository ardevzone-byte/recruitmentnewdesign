...
class Candidates extends CI_Controller {
   ...
   public function add_manual() {
    ...
    // Add this to Candidates.php

    public function export_excel() {
        // Security check
        $user_id = $this->session->userdata('username');
        if (!in_array($user_id, ['1526', '1291','2200','2439']) && $this->session->userdata('role') != 'recruitment_manager') {
            redirect('dashboard');
        }
        ...
        if (empty($app_ids)) {
            $this->session->set_flashdata('error_msg', 'الرجاء تحديد مرشح واحد على الأقل للتصدير.');
            redirect('candidates/archive');
            return;
        }

        // Fetch data from Model
        $export_data = $this->candidate_model->get_export_data($app_ids);
        ...
        // Add BOM to fix UTF-8 Arabic characters in Excel
        fputs($f, "\xEF\xBB\xBF");

        // Add Excel Headers
        $headers = [
            'Employee ID', 'Name', 'ID Number', 'Gender', 'Phone', 'Job title', 'Email', 
            'Date of Birth (Hijri)', 'Date of Birth (Gregorian)', 'Date of joining', 
            'Branch', 'Iban', 'Bank name', 'Qualification', 'Specialization', 
            'Basic', 'Housing Allowance', 'Transport allowance', 'Other', 'Total'
        ];
        fputcsv($f, $headers);
        ...
        foreach ($export_data as $row) {
            // Convert Dates
            $dates = $this->_process_dates($row['date_of_birth']);
            ...
            $line = [
                ...
                $row['full_name'] ?? '',
                $row['id_number'] ?? '',
                '', // Gender (Not explicitly selected in DB schema, manually added placeholder)
                $row['phone'] ?? '',
                $row['job_title'] ?? '',
                $row['email'] ?? '',
                $dates['hijri'],
                ...
                $row['date_of_joining'] ?? '',
                $row['branch'] ?? '',
                $row['iban'] ? 'Uploaded' : 'No File', // Actual IBAN text isn't in schema, shows if file exists
    ...
    private function _process_dates($date) {
        $result = ['hijri' => '', 'gregorian' => ''];
        if (empty($date) || $date == '0000-00-00') return $result;

        $parts = explode('-', $date);
        if (count($parts) != 3) {
            $result['gregorian'] = $date; // Fallback
            return $result;
        }
        ...
        // Requires PHP Calendar extension to be enabled
        if (function_exists('gregoriantojd') && function_exists('islamictojd')) {
            if ($year > 1800) { 
                // It is Gregorian
                $result['gregorian'] = $date;
                $jd = gregoriantojd($month, $day, $year);
                $hijri_str = jdtoislamic($jd); // Returns m/d/y
                $h_parts = explode('/', $hijri_str);
                $result['hijri'] = sprintf('%04d-%02d-%02d', $h_parts[2], $h_parts[0], $h_parts[1]);
            } else {
                // It is Hijri
                $result['hijri'] = $date;
                $jd = islamictojd($month, $day, $year);
                $greg_str = jdtogregorian($jd); // Returns m/d/y
                $g_parts = explode('/', $greg_str);
                $result['gregorian'] = sprintf('%04d-%02d-%02d', $g_parts[2], $g_parts[0], $g_parts[1]);
            }
        } else {
            // Fallback if server doesn't have Calendar ext
            $year > 1800 ? $result['gregorian'] = $date : $result['hijri'] = $date;
        }
    ...
    $company = ($this->input->post('is_dr_saleh_office') == '1')
    ...
    $this->db->group_start();
    if(!empty($candidate_data['email'])) $this->db->where('email', $candidate_data['email']);
    if(!empty($candidate_data['phone'])) $this->db->or_where('phone', $candidate_data['phone']);
    $this->db->group_end();
    ...
    $app_exists = $this->db->get_where('applications', [
...
public function profile($candidate_id) {
    ...
    $data['education'] = $this->db->get_where('candidate_education', ['candidate_id' => $candidate_id])->result_array();
    $data['experience'] = $this->db->get_where('candidate_experience', ['candidate_id' => $candidate_id])->result_array();
 ...
 $this->load->view('template/new_header', $data);
   ...
   $this->load->view('template/new_footer');
            ...
            <div class="col-md-6 mx-auto">
                <div class="card shadow">
                    <div class="card-header bg-success text-white">
                        <h5 class="mb-0"><i class="fas fa-link"></i> ربط المرشح: '.$data['candidate']['full_name'].'</h5>
                    </div>
                    <div class="card-body">
    ...
    public function submit_link_job() {
        ...
        $exists = $this->db->get_where('applications', [
...
public function archive() {
        ...
        $search_query = $this->input->post('search_query') ? $this->input->post('search_query', TRUE) : $this->input->get('search_query', TRUE);
        $status_filter = $this->input->post('status_filter') ? $this->input->post('status_filter', TRUE) : $this->input->get('status_filter', TRUE);
...
public function process_schedule() {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            if ($info) {
                ...
                $this->session->set_flashdata('success_msg', 'تم جدولة المقابلة (لم يتم العثور على بيانات المرشح للإرسال).');
            }
        }
...
public function send_form_invitation($application_id) {
        // Security Check
        $current_user = $this->session->userdata('username');
        if(($current_user != '1526' && $current_user != '2200' && $current_user != '2439') && $this->session->userdata('role') != 'ecruitment_manager') {
...
public function request_evaluation() {
        ...
        if (!empty($manager_ids)) {
            if (!is_array($manager_ids)) $manager_ids = [$manager_ids];
            ...
            foreach ($manager_ids as $mid) {
                ...
                if($this->evaluation_model->create_request($data)) $count++;
            }
...
public function my_pending_evaluations() {
        ...
        foreach ($tasks as &$task) {
            $salary_data = $this->db->select('recommended_salary')
  ...
  public function submit_evaluation_result() {
        ...
        // --- NEW: Handle multiple checkboxes ---
        $reasons_array = $this->input->post('evaluation_reason');
        $reasons_string = '';
        
        // If reasons were checked, combine them into one string separated by " - "
        if (!empty($reasons_array) && is_array($reasons_array)) {
        ...
        $data = [
            ...
            'evaluation_reason' => $reasons_string, // <-- Saves the combined string
            'notes'             => $this->input->post('notes'),
            'status'            => 'completed',
            'completed_at'      => date('Y-m-d H:i:s')
        ];

        // Capture Recommended Salary if it exists
