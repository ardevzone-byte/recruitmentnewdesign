...
class Dashboard extends CI_Controller {
    ...
    public function index() {
        ...
        if ($role == 'recruitment_manager') {
            $stats['pending_requisitions'] = count($this->requisition_model->get_requisitions_by_status('بانتظار مدير التوظيف'));
        } elseif ($role == 'ceo') {
            $stats['pending_requisitions'] = count($this->requisition_model->get_requisitions_by_status('بانتظار الرئيس التنفيذي'));
        } else {
    ...
    try {
        ...
        if (!empty($candidates)) {
            ...
            foreach ($candidates as $index => $candidate) {
                ...
                $html .= '<td dir="ltr" class="text-end">' . ($candidate['phone'] ?? '<span class="text-muted">-</span>') . '</td>';
                ...
                if ($app_id) {
                    ...
                    $html .= '<a href="' . base_url('candidates/view/' . $app_id) . '" class="btn btn-outline-primary fw-bold btn-sm" title="عرض الملف">';
                    $html .= '<i class="fas fa-eye"></i> الملف';
                    $html .= '</a>';
                } else {
                    // If no application exists, show profile link as fallback
                    $html .= '<a href="' . base_url('candidates/profile/' . $candidate['id']) . '" class="btn btn-info btn-sm" title="الملف الشخصي">';
                    $html .= '<i class="fas fa-user"></i>';
                    $html .= '</a>';
                }
                
                // Link to archive
                $html .= '<a href="' . base_url('candidates/archive?search_query=' . urlencode($candidate['full_name'] ?? '')) . '" class="btn btn-secondary btn-sm" title="البحث في الأرشيف">';
